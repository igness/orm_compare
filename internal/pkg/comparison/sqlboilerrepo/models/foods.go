...
var FoodWhere = struct {
	...
	ID:        whereHelperuint64{field: "`foods`.`id`"},
	CreatedAt: whereHelpertime_Time{field: "`foods`.`created_at`"},
	UpdatedAt: whereHelpertime_Time{field: "`foods`.`updated_at`"},
	Name:      whereHelperstring{field: "`foods`.`name`"},
	Price:     whereHelpertypes_Decimal{field: "`foods`.`price`"},
}
...
var (
	foodAllColumns            = []string{"id", "created_at", "updated_at", "name", "price"}
	foodColumnsWithoutDefault = []string{"created_at", "updated_at", "name", "price"}
	foodColumnsWithDefault    = []string{"id"}
	foodPrimaryKeyColumns     = []string{"id"}
)

type (
	// FoodSlice is an alias for a slice of pointers to Food.
	// This should generally be used opposed to []Food.
	FoodSlice []*Food
...
func (q foodQuery) One(ctx context.Context, exec boil.ContextExecutor) (*Food, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: failed to execute a one query for foods")
	}
...
func (q foodQuery) All(ctx context.Context, exec boil.ContextExecutor) (FoodSlice, error) {
	...
	err := q.Bind(ctx, exec, &o)
	if err != nil {
		return nil, errors.Wrap(err, "models: failed to assign all query results to Food slice")
	}
...
func (q foodQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to count foods rows")
	}
...
func (q foodQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "models: failed to check if foods exists")
	}
...
func Foods(mods ...qm.QueryMod) foodQuery {
	mods = append(mods, qm.From("`foods`"))
	return foodQuery{NewQuery(mods...)}
}

// FindFood retrieves a single record by ID with an executor.
// If selectCols is empty Find will return all columns.
func FindFood(ctx context.Context, exec boil.ContextExecutor, iD uint64, selectCols ...string) (*Food, error) {
	foodObj := &Food{}
	...
	query := fmt.Sprintf(
		"select %s from `foods` where `id`=?", sel,
	)
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: unable to select from foods")
	}
...
func (o *Food) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("models: no foods provided for insertion")
	}
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO `foods` (`%s`) %%sVALUES (%s)%%s", strings.Join(wl, "`,`"), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO `foods` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `foods` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, foodPrimaryKeyColumns))
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to insert into foods")
	}
	...
	o.ID = uint64(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == foodMapping["id"] {
		goto CacheNoHooks
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for foods")
	}
...
func (o *Food) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("models: unable to update foods, could not build whitelist")
		}

		cache.query = fmt.Sprintf("UPDATE `foods` SET %s WHERE %s",
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update foods row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by update for foods")
	}
...
func (q foodQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update all for foods")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to retrieve rows affected for foods")
	}
...
func (o FoodSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE `foods` SET %s WHERE %s",
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update all in food slice")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to retrieve rows affected all in update all food")
	}
	return rowsAff, nil
}
...
func (o *Food) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("models: no foods provided for upsert")
	}
	if !boil.TimestampsAreSkipped(ctx) {
		currTime := time.Now().In(boil.GetLocation())
	...
	if !cached {
		...
		if len(update) == 0 {
			return errors.New("models: unable to upsert foods, could not build update column list")
		}

		ret = strmangle.SetComplement(ret, nzUniques)
		cache.query = buildUpsertQueryMySQL(dialect, "foods", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `foods` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for foods")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for foods")
	}
...
func (o *Food) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if o == nil {
		return 0, errors.New("models: no Food provided for delete")
	}
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete from foods")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by delete for foods")
	}
...
func (q foodQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if q.Query == nil {
		return 0, errors.New("models: no foodQuery provided for delete all")
	}
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete all from foods")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by deleteall for foods")
	}
...
func (o FoodSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete all from food slice")
	}
...
func (o *Food) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindFood(ctx, exec, o.ID)
	if err != nil {
		return err
	}
...
func (o *FoodSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT `foods`.* FROM `foods` WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, foodPrimaryKeyColumns, len(*o))
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to reload all in FoodSlice")
	}
...
func FoodExists(ctx context.Context, exec boil.ContextExecutor, iD uint64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `foods` where `id`=? limit 1)"
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "models: unable to check if foods exists")
	}
